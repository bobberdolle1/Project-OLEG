...
from app.services.voice_recognition import transcribe_voice_message, transcribe_video_note, is_available
...
async def handle_video_note(msg: Message):
    ...
    Извлекает аудио и распознаёт речь как голосовое.
    """
    # Проверяем, включена ли функция
    if not settings.voice_recognition_enabled:
        await msg.reply("Кружочки? Пиши текстом, я не твоя мамка.")
        return
    ...
    # Отправляем индикатор обработки
    processing_msg = await msg.reply("🎥 Слушаю твой кружочек...")
    
    try:
        # Распознаём аудио из видеосообщения
        text = await transcribe_video_note(msg.bot, msg.video_note.file_id)
        
        if not text:
            await processing_msg.edit_text(
                "Не разобрал, что ты там бормочешь в кружочке. "
                "Говори чётче или пиши текстом."
            )
            return
        ...
        logger.info(f"Видеосообщение от @{msg.from_user.username}: {text[:50]}...")
        
    except Exception as e:
        logger.error(f"Ошибка при обработке видеосообщения: {e}")
        await processing_msg.edit_text(
            "Что-то пошло не так с твоим кружочком. "
            "Попробуй ещё раз или пиши текстом."
        )
