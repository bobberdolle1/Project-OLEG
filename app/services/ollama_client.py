...
ДЛИНА ОТВЕТА: Адаптируй под ситуацию как реальный человек!
• Простой вопрос/реплика → 1-2 предложения ("это мой дек" → "Ок, ошибся")
• Средний вопрос → 2-3 предложения
• Сложная тема (настройка, тайминги) → 4-5 предложений, суть без воды
• НИКОГДА не пиши больше 5 предложений — это уже не человек, а бот
...
Используй эту информацию о ссылках чтобы понять контекст. Если человек скинул YouTube видео — ты знаешь название и автора. Если веб-страницу — знаешь о чём она."""
    ...
    try:
        if is_fallback_model and tool_model:
            ...
            response = await _ollama_chat(messages, model=tool_model, enable_tools=True, final_model=active_model, num_predict=250)
        else:
            # Основная модель поддерживает tools — используем её для всего
            response = await _ollama_chat(messages, model=active_model, enable_tools=not is_fallback_model, num_predict=250)
        ...
        if settings.ollama_fallback_enabled:
            ...
            try:
                ...
                if tool_model:
                    ...
                    response = await _ollama_chat(messages, model=tool_model, enable_tools=True, final_model=fallback_chat, num_predict=250)
                    return response
                else:
                    # Нет tool_model — используем fallback без tools
                    return await _ollama_chat(messages, model=fallback_chat, enable_tools=False, num_predict=250)
